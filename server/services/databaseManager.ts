...
export default class DatabaseManager {
  ...
  async initialize() {
    try {
      ...
      console.log(`⚠️ MySQL initialization failed: ${error.message}`);
      return { success: false, provider: 'mysql', error: error.message };
    }
  }
  ...
  async connectToMySQL() {
    try {
      ...
      // We don't throw here to allow the app to work with fallbacks
    }
  }
  ...
  async getAllServers() {
    ...
    const servers = await this.query(sql) as any[];
  ...
  async getServer(serverId: string) {
    ...
    const results = await this.query(sql, [serverId]) as any[];
  ...
  async updateServer(serverId: string, updates: Record<string, any>) {
  ...
  async addCategory(data: Record<string, any>) {
    const sql = `
  ...
  async getAllGroups() {
    ...
    const groups = await this.query(sql) as any[];
  ...
  async addGroup(data: Record<string, any>) {
    const sql = `
